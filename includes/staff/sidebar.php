...
<aside
    class="dashboard-sidebar offcanvas-lg offcanvas-start bg-white border-end d-flex flex-column min-vh-100"
    style="width: 264px; min-width: 264px; max-width: 264px; flex-shrink: 0"
    tabindex="-1"
    id="sidebarOffcanvas"
>
    <div
        class="dashboard-sidebar-brand d-flex align-items-center justify-content-between gap-2 border-bottom px-3 py-3"
    >
    ...
    <nav class="dashboard-sidebar-scroll flex-grow-1 py-3 px-3">
        ...
        <ul class="dashboard-sidebar-menu nav flex-column mb-4">
            <li class="nav-item mb-1">
                <a
                    href="dashboard.php"
                    class="nav-link <?= navActive('dashboard.php', $currentPage) ?> d-flex align-items-center gap-3 rounded-3 px-3 py-2"
                >
        ...
        <ul class="dashboard-sidebar-menu nav flex-column mb-4">
            <li class="nav-item mb-1">
                <a
                    href="my_assignments.php"
                    class="nav-link <?= navActive('my_assignments.php', $currentPage) ?> d-flex align-items-center flex-nowrap gap-3 rounded-3 px-3 py-2"
                >
                    ...
                    <span class="d-flex align-items-center gap-1 flex-shrink-0 sidebar-badge-group">
                        ...
                        <span
                            class="sidebar-badge sidebar-badge-new"
                            title="<?= $newAssignmentCount ?> new assignment<?= $newAssignmentCount === 1 ? '' : 's' ?>"
                        >
                            <?= badgeCount($newAssignmentCount) ?>
                        </span>
                        <?php endif; ?> <?php if ($progressAssignmentCount > 0): ?>
                        <span
                            class="sidebar-badge sidebar-badge-progress"
                            title="<?= $progressAssignmentCount ?> in progress"
                        >
    ...
    <div class="dashboard-sidebar-footer border-top p-3">
        <div class="dropdown">
            <button
                type="button"
                class="btn btn-link p-0 w-100 text-start text-decoration-none dropdown-toggle d-flex align-items-center gap-3 text-dark"
                data-bs-toggle="dropdown"
                aria-expanded="false"
            >
                <span
                    class="dashboard-user-icon d-flex align-items-center justify-content-center rounded-circle bg-primary bg-opacity-10 flex-shrink-0"
                ...
                <div class="overflow-hidden flex-grow-1">
                    ...
                    <div class="text-secondary text-truncate" style="font-size: 0.75rem">
                        <?= htmlspecialchars(maskEmail($staffEmail)) ?>
                    </div>
                </div>
            </button>
            <ul class="dropdown-menu dropdown-menu-end w-100 shadow-sm">
                <li>
                    <a
                        href="../config/logout.php?role=staff"
