...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'upload') {
        ...
        if (!$doc || $doc['uploaded_by_role'] !== 'staff' || (int) $doc['uploaded_by'] !== (int) $currentUserId) {
            $_SESSION['alert_type'] = 'error';
...
<title>Knowledge Repository | Staff</title>
          ...
          <ul class="document-list list-unstyled" id="documentListUl">
            <?php foreach ($documents as $doc): ?>
              <?php
                ...
                $isOwner = $doc['uploaded_by_role'] === 'staff' && (int) $doc['uploaded_by'] === (int) $currentUserId;
              ?>
