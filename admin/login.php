...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare('SELECT admin_id, fullname, email, password FROM administrator WHERE email = ?');
    $stmt->execute([$email]);
    $admin = $stmt->fetch();
  ...
  <section class="login-card card shadow-sm border-0 rounded-4">
    <div class="login-card-body card-body p-4 p-md-5">
      ...
      <header class="login-header mb-4">
        ...
        <p class="login-subtitle text-secondary small mb-0">Please sign in to your administrator account</p>
      </header>
      ...
      <form class="login-form" action="login.php" method="POST" novalidate>
        ...
        <div class="login-options d-flex justify-content-between align-items-center mb-4">
          ...
          <a href="forgot_password.php" class="login-forgot-link link-primary small text-decoration-none">Forgot password?</a>
        </div>
